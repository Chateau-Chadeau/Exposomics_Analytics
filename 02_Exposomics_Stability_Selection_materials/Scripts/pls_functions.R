...
CalibratesPLSDA=function(dataX, dataY, ncomp=1, Nrepeat=100, name="1", xseq=NULL){
  ...
  for (comp in 1:ncomp){
    for(i in xseq){
      ...
      TmpsPLS <- splsda(dataX,dataY,keepX=TmpKeepX,ncomp=comp)
      error=NULL
      print(i)
      ...
      for (k in 1:Nrepeat){
        ...
        for (l in 1:length(folds)){
          ...
          sPLS_train <- splsda(dataX_train,dataY_train,keepX=TmpKeepX,ncomp=comp)
...
StabilityPlot=function(X, Y, NIter=100, plot=TRUE){
  ...
  for (NVar in seq((ncol(X)-1), 1, by=-1)){
    ...
    for (k in 1:NIter){
      ...
      TmpsPLS=splsda(X_sub, Y_sub, keepX=NVar, ncomp=1)
